...
const emptyData = () => ({
  plants: [], // { id, name }
  entries: [], // { id, number, plantId, date, symptomText, causes:[], actionText, resultText, historyActionsText, recommendedActionsText, avoidActionsText, memo, photo, createdAt, updatedAt }
});
...
// 一致した過去の記録から、原因・対策・良かったこと・避けた方がよいことを集計する
function aggregatePhotoMatches(matches) {
...
function EntryForm({ data, entry, defaultPlantName, onSave, onCancel, onDelete }) {
  ...
  const [historyActionsText, setHistoryActionsText] = useState(entry?.historyActionsText || "");
  ...
  const runPhotoMatch = async (dataUrl) => {
    ...
    try {
      ...
      if (best >= 80) {
        const agg = aggregatePhotoMatches(scored);
        setHistoryActionsText(agg.actions.length > 0 ? agg.actions.join("\n") : UNKNOWN);
        setRecommendedActionsText(agg.goodActions.length > 0 ? agg.goodActions.join("\n") : UNKNOWN);
        setAvoidActionsText(agg.badActions.length > 0 ? agg.badActions.join("\n") : UNKNOWN);
      } else {
        setHistoryActionsText(UNKNOWN);
        setRecommendedActionsText(UNKNOWN);
        setAvoidActionsText(UNKNOWN);
      }
  ...
  return (
    <div className="eo-form">
      <div className="eo-field">
        ...
        {autoFilledFromPhoto && !photoMatchLoading && (
          <div className="eo-related-hint" style={{ marginTop: 8 }}>
            過去の写真との照合結果を「これまで試した対策」以下の欄に自動で入れました（編集できます）。
          </div>
        )}
      </div>
      ...
      <div className="eo-field">
        <div className="eo-field-label">これまで試した対策</div>
        <div className="eo-related-hint">写真を追加すると、一致度80%以上の過去の記録があれば自動で入ります（無ければ unknown）。</div>
        <textarea
          className="eo-textarea"
          rows={2}
          value={historyActionsText}
          onChange={(e) => setHistoryActionsText(e.target.value)}
          placeholder="unknown"
        />
      </div>
...
function DiagnoseScreen({ data }) {
  ...
  const aggregate = useMemo(() => aggregatePhotoMatches(matches), [matches]);
          ...
          <div className="eo-diag-panel">
            ...
            {aggregate.goodActions.length > 0 ? (
              ...
              <>
                ...
                <div className="eo-diag-unknown">
                  <HelpCircle size={15} style={{ marginRight: 6, flexShrink: 0 }} />
                  一致度80%以上で、結果が良かった記録がないため、分かりません。
                </div>
              </>
            )}
            ...
            {aggregate.badActions.length > 0 ? (
              ...
              <>
                ...
                <div className="eo-diag-unknown">
                  <HelpCircle size={15} style={{ marginRight: 6, flexShrink: 0 }} />
                  一致度80%以上で、結果が悪かった記録がないため、分かりません。
                </div>
              </>
            )}
          </div>
  ...
  const handleSaveEntry = (payload, editingId) => {
    ...
    if (editingId) {
      ...
      savedEntry = {
        ...
        historyActionsText: payload.historyActionsText,
