...
const emptyData = () => ({
  plants: [], // { id, name }
  entries: [], // { id, number, plantId, date, symptomText, actionText, researchedInfoText, causes:[], goodResultText, badResultText, recommendedActionsText, avoidActionsText, memo, photo, createdAt, updatedAt }
});
...
function entryFreeText(e) {
  return [e.symptomText, e.actionText, e.goodResultText, e.badResultText, e.memo].filter(Boolean).join(" ");
}
...
// 写真1枚を過去の記録の写真と比較し、一致度が高い順に上位を返す（APIなし・常に動く）
async function computePhotoMatches(dataUrl, entries, excludeId, limit = 3) {
  const targetHash = await computeImageHash(dataUrl);
  const withPhotos = entries.filter((en) => en.photo && en.id !== excludeId);
  const scored = [];
...
// 特定の原因を持つ過去の記録から、対策・良かったこと・避けた方がよいことを集計する
// （写真の一致度とは無関係に、その原因が記録されている記録すべてを対象にする）
function buildCauseRecommendations(entries, causeLabel, excludeId) {
  const actionsAll = new Set();
  const goodActions = new Set();
  const badActions = new Set();
  entries
    .filter((e) => e.id !== excludeId && (e.causes || []).includes(causeLabel))
    .forEach((e) => {
      if (!e.actionText) return;
      actionsAll.add(e.actionText);
      if (e.goodResultText) goodActions.add(e.actionText);
      if (e.badResultText) badActions.add(e.actionText);
    });
  return {
...
// 一致した過去の記録から原因の候補を出し、その中で一番可能性の高い原因をもとに
// 対策・良かったこと・避けた方がよいことを（写真の一致度に関わらず）集計する
function aggregatePhotoMatches(matches, allEntries, excludeId) {
  ...
  const topCause = causes[0]?.label;
  const recs = topCause ? buildCauseRecommendations(allEntries, topCause, excludeId) : { actions: [], goodActions: [], badActions: [] };

  return {
    causes,
    actions: recs.actions,
    goodActions: recs.goodActions,
    badActions: recs.badActions,
  };
}
...
function EntryForm({ data, entry, defaultPlantName, onSave, onCancel, onDelete }) {
  ...
  const [researchedInfoText, setResearchedInfoText] = useState(entry?.researchedInfoText || "");
  ...
  const currentFreeText = [symptomText, actionText, entry?.goodResultText, entry?.badResultText, memo].filter(Boolean).join(" ");
  ...
  return (
    <div className="eo-form">
      ...
      <div className="eo-field">
        <div className="eo-field-label">調べた情報</div>
        <div className="eo-related-hint">URLを貼っても、自由に文章で書いてもOKです（URLの中身の自動取得はできません）。</div>
        <textarea
          className="eo-textarea"
          rows={3}
          value={researchedInfoText}
          onChange={(e) => setResearchedInfoText(e.target.value)}
          placeholder="本やサイトで調べたこと、参考にしたURLなど"
        />
      </div>
          ...
          <div className="eo-diag-similar-list">
            {relatedEntries.map(({ entry: e }) => (
              <div className="eo-diag-similar-item" key={e.id}>
                ...
                {e.goodResultText && <span className="eo-diag-similar-result">良かった結果: {e.goodResultText}</span>}
                {e.badResultText && <span className="eo-diag-similar-result">悪かった結果: {e.badResultText}</span>}
              </div>
            ))}
          </div>
        </div>
...
function DiagnoseScreen({ data }) {
  ...
  const handlePhoto = async (e) => {
    ...
    try {
      const dataUrl = await compressImage(file);
      setPhoto(dataUrl);
      const scored = await computePhotoMatches(dataUrl, data.entries, null, MAX_RESULTS);
      setMatches(scored);
    } catch (err) {
  ...
  const aggregate = useMemo(() => aggregatePhotoMatches(matches, data.entries, null), [matches, data.entries]);

  return (
    <div className="eo-diagnose">
      <div className="eo-diagnose-intro">
        写真を1枚選ぶと、これまでの記録の中から見た目が近い写真を、一致度が高い順に最大{MAX_RESULTS}件表示します。
        一致度が低い場合もそのまま表示されるので、参考程度にご覧ください。
        対策・これからやればいいこと・してはいけないことは、一番可能性の高い原因と同じ原因が記録されている過去の記録すべてから作ります。APIやネット接続は使いません。
      </div>
          ...
          <div className="eo-diag-panel">
            ...
            {aggregate.goodActions.length > 0 ? (
              ...
              <>
                ...
                <div className="eo-diag-unknown">
                  <HelpCircle size={15} style={{ marginRight: 6, flexShrink: 0 }} />
                  同じ原因の記録の中に、結果が良かったものがないため、分かりません。
                </div>
              </>
            )}
            ...
            {aggregate.badActions.length > 0 ? (
              ...
              <>
                ...
                <div className="eo-diag-unknown">
                  <HelpCircle size={15} style={{ marginRight: 6, flexShrink: 0 }} />
                  同じ原因の記録の中に、結果が悪かったものがないため、分かりません。
                </div>
              </>
            )}
          </div>
...
const TABS = [
  ...
  { key: "results", label: "結果", icon: ClipboardCheck },
];
...
export default function ExperienceOS() {
  ...
  const [resultsOnlyMissing, setResultsOnlyMissing] = useState(true);
  ...
  const handleSaveEntry = (payload, editingId) => {
    ...
    if (editingId) {
      ...
      savedEntry = {
        ...
        causes: [], // 原因は下部タブ「結果」から後で記録する
        actionText: payload.actionText,
        researchedInfoText: payload.researchedInfoText,
        recommendedActionsText: "", // これからやればいいこと／してはいけないことも「結果」から後で記録する
        avoidActionsText: "",
        goodResultText: "", // 結果は下部タブ「結果」から後で記録する
        badResultText: "",
        memo: payload.memo,
        photo: payload.photo,
        createdAt: now,
  ...
  const handleSaveResult = (entryId, payload) => {
    const now = new Date().toISOString();
    let savedEntry;
    const entries = data.entries.map((e) => {
      if (e.id !== entryId) return e;
      savedEntry = { ...e, ...payload, updatedAt: now };
      return savedEntry;
    });
    setData({ ...data, entries });
    setView({ name: "results" });
    if (savedEntry) saveEntryToStorage(savedEntry).catch(() => showSaveError());
  };
  ...
  const searchResults = searching
    ? data.entries
        .filter((e) => {
          ...
          const haystack = [plant?.name, e.symptomText, ...(e.causes || []), e.actionText, e.goodResultText, e.badResultText, e.memo]
            .filter(Boolean)
            .join(" ")
            .toLowerCase();
  ...
  const activeTab =
    view.name === "diagnose"
      ...
      : view.name === "records"
      ? "records"
      : view.name === "results" || view.name === "resultDetail"
      ? "results"
      : "home";

  const showHeader = view.name !== "home" && view.name !== "diagnose" && view.name !== "records" && view.name !== "results";
  const headerTitle =
    view.name === "plant"
      ? currentPlant?.name
      : view.name === "new"
  ...
  return (
    <div className="eo-root">
            ...
            <div className="eo-brand">
              ...
              <div>
                <div className="eo-brand-title">
                  {view.name === "diagnose" ? "画像診断" : view.name === "records" ? "記録一覧" : view.name === "results" ? "結果を記録" : "観察記録"}
                </div>
      ...
      <main className="eo-main">
        ...
        {view.name === "results" &&
          (() => {
            const list = resultsOnlyMissing
              ? allEntriesByDate.filter((e) => !e.goodResultText && !e.badResultText)
              : allEntriesByDate;
            return (
              <>
                <button
                  type="button"
                  className="eo-diag-add-btn"
                  style={{ marginBottom: 14 }}
                  onClick={() => setResultsOnlyMissing((v) => !v)}
                >
                  {resultsOnlyMissing ? "未記入のみ表示中（タップで全件表示）" : "全件表示中（タップで未記入のみ）"}
                </button>
                {list.length === 0 ? (
                  <EmptyState
                    text={
                      resultsOnlyMissing
                        ? "結果が未記入の記録はありません。"
                        : "まだ記録がありません。下の「追加」から最初の観察を記録してみましょう。"
                    }
                  />
                ) : (
                  <div className="eo-card-grid">
                    {list.map((e) => (
                      <SpecimenCard
                        ...
                        entry={e}
                        plantName={data.plants.find((p) => p.id === e.plantId)?.name || ""}
                        onOpen={(id) => setView({ name: "resultDetail", entryId: id })}
                      />
                    ))}
                  </div>
                )}
          ...
          (() => {
            ...
            return (
              <ResultDetailForm
                ...
                data={data}
                entry={entry}
                plantName={data.plants.find((p) => p.id === entry.plantId)?.name || ""}
                onCancel={() => setView({ name: "results" })}
                onSave={(payload) => handleSaveResult(entry.id, payload)}
              />
            );
          })()}
...
function ResultDetailForm({ data, entry, plantName, onSave, onCancel }) {
  const [causes, setCauses] = useState(entry.causes || []);
  const [goodResultText, setGoodResultText] = useState(entry.goodResultText || "");
  const [badResultText, setBadResultText] = useState(entry.badResultText || "");
  const [recommendedActionsText, setRecommendedActionsText] = useState(entry.recommendedActionsText || "");
  const [avoidActionsText, setAvoidActionsText] = useState(entry.avoidActionsText || "");

  const [matchLoading, setMatchLoading] = useState(false);
  const [matchError, setMatchError] = useState("");
  const [bestMatch, setBestMatch] = useState(null); // { entry, score } 参考表示専用（編集不可）

  // 「結果」画面を開いたら、保存済みの写真を使って自動で一番一致度が高い過去の記録を探し、
  // まだ何も記録されていない場合にだけ、原因・これからやればいいこと・してはいけないことを自動で入れる。
  // 一度手で編集・保存したものを、開き直すたびに上書きしないようにするための条件。
  useEffect(() => {
    if (!entry.photo) return;
    const alreadyFilled = (entry.causes && entry.causes.length > 0) || entry.recommendedActionsText || entry.avoidActionsText;
    let cancelled = false;
    (async () => {
      setMatchLoading(true);
      setMatchError("");
      try {
        const scored = await computePhotoMatches(entry.photo, data.entries, entry.id, 1);
        if (cancelled) return;
        const top = scored[0] || null;
        setBestMatch(top || null);
        if (top && !alreadyFilled) {
          const topCauseLabel = (top.entry.causes || [])[0];
          if (topCauseLabel) {
            setCauses((c) => (c.length > 0 ? c : [topCauseLabel]));
            const recs = buildCauseRecommendations(data.entries, topCauseLabel, entry.id);
            setRecommendedActionsText((v) => (v ? v : recs.goodActions.join("\n")));
            setAvoidActionsText((v) => (v ? v : recs.badActions.join("\n")));
          }
        }
      } catch (err) {
        if (!cancelled) setMatchError("写真の照合に失敗しました。");
      } finally {
  ...
  const currentFreeText = [entry.symptomText, entry.actionText, goodResultText, badResultText, entry.memo]
    .filter(Boolean)
    .join(" ");

  const suggestedCauses = useMemo(
    () => suggestCauses(currentFreeText, data.entries, entry.id),
    [currentFreeText, data.entries, entry.id]
  );
  ...
  return (
    <div className="eo-form">
      <div className="eo-field">
        <div className="eo-related-hint">
          {plantName} ・ No.{String(entry.number).padStart(3, "0")} ・ {formatDateJP(entry.date)}
        </div>
        {entry.symptomText && <div className="eo-card-memo" style={{ marginTop: 4 }}>症状: {entry.symptomText}</div>}
        {entry.actionText && <div className="eo-card-memo">試したこと: {entry.actionText}</div>}
      </div>

      {matchLoading && (
        <div className="eo-diagnose-loading">
          <Loader2 size={15} className="eo-spin" />
          過去の写真と照合しています…
        </div>
      )}
      {matchError && <div className="eo-error">{matchError}</div>}

      {!matchLoading && bestMatch && (bestMatch.entry.goodResultText || bestMatch.entry.badResultText) && (
        <div className="eo-field">
          <div className="eo-field-label">これまで試した結果（参考・一番近い記録）</div>
          <div className="eo-diag-similar-item">
            <span className="eo-diag-match-badge">{bestMatch.score}%一致</span>
            <span className="eo-diag-similar-num">No.{String(bestMatch.entry.number).padStart(3, "0")}</span>
            {bestMatch.entry.goodResultText && (
              <span className="eo-diag-similar-result">良かった結果: {bestMatch.entry.goodResultText}</span>
            )}
            {bestMatch.entry.badResultText && (
              <span className="eo-diag-similar-result">悪かった結果: {bestMatch.entry.badResultText}</span>
            )}
          </div>
        </div>
      )}
      ...
      <div className="eo-field">
        <div className="eo-field-label">これからやればいいこと</div>
        <textarea
          className="eo-textarea"
          rows={2}
      ...
      <div className="eo-field">
        <div className="eo-field-label">してはいけないこと</div>
        <textarea
          className="eo-textarea"
          rows={2}
          value={avoidActionsText}
          onChange={(e) => setAvoidActionsText(e.target.value)}
          placeholder="unknown"
        />
      </div>

      <div className="eo-field">
        <div className="eo-field-label">良かった結果</div>
        <textarea
          className="eo-textarea"
          rows={3}
          value={goodResultText}
          onChange={(e) => setGoodResultText(e.target.value)}
          placeholder="うまくいったことを自由に書いてください（例：3日ほどで葉に元気が戻った）"
        />
      </div>

      <div className="eo-field">
        <div className="eo-field-label">悪かった結果</div>
        <textarea
          className="eo-textarea"
          rows={3}
          value={badResultText}
          onChange={(e) => setBadResultText(e.target.value)}
          placeholder="うまくいかなかったことを自由に書いてください（例：葉が数枚さらに落ちた）"
        />
      </div>

      <div className="eo-related-hint">結果はどちらか一方だけの記入でも構いません。</div>

      <div className="eo-form-actions">
        <button type="button" className="eo-btn eo-btn-ghost" onClick={onCancel}>
          キャンセル
        </button>
        <button type="button" className="eo-btn eo-btn-primary" onClick={() => onSave({ causes, goodResultText, badResultText, recommendedActionsText, avoidActionsText })}>
          保存する
        </button>
      </div>
    </div>
...
function StyleSheet() {
  return (
    <style>{`
      ...
      .eo-search input { border: none; outline: none; background: transparent; flex: 1; font-size: 16px; color: var(--ink); font-family: 'Work Sans', sans-serif; }
      ...
      .eo-input, .eo-textarea {
        ...
        padding: 10px 12px; font-size: 16px; color: var(--ink); font-family: 'Work Sans', sans-serif;
      ...
      .eo-chip-add input { border: none; outline: none; background: transparent; font-size: 16px; width: 108px; font-family: 'Work Sans', sans-serif; }
