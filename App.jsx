...
function EntryForm({ data, entry, defaultPlantName, onSave, onCancel, onDelete }) {
  ...
  const [autoFilledFromPhoto, setAutoFilledFromPhoto] = useState(false);
  ...
  const [photoMatchLoading, setPhotoMatchLoading] = useState(false);
  ...
  const runPhotoMatch = async (dataUrl) => {
    setPhotoMatchLoading(true);
    setPhotoMatchError("");
    try {
      const scored = await computePhotoMatches(dataUrl, data.entries, entry?.id, 3);
      const agg = aggregatePhotoMatches(scored, data.entries, entry?.id);
      setRecommendedActionsText(agg && agg.goodActions.length > 0 ? agg.goodActions.join("\n") : UNKNOWN);
      setAvoidActionsText(agg && agg.badActions.length > 0 ? agg.badActions.join("\n") : UNKNOWN);
      setAutoFilledFromPhoto(true);
    } catch (err) {
      setPhotoMatchError("写真の照合に失敗しました。");
    } finally {
      setPhotoMatchLoading(false);
    }
  };
  ...
  return (
    <div className="eo-form">
      <div className="eo-field">
        ...
        {photoMatchLoading && (
          <div className="eo-diagnose-loading" style={{ marginTop: 10 }}>
            <Loader2 size={15} className="eo-spin" />
            過去の写真と照合しています…
          </div>
        )}
        {photoMatchError && <div className="eo-error">{photoMatchError}</div>}
        {autoFilledFromPhoto && !photoMatchLoading && (
          <div className="eo-related-hint" style={{ marginTop: 8 }}>
            過去の写真との照合結果を「これからやればいいこと」以下の欄に自動で入れました（編集できます）。
          </div>
        )}
      </div>
  ...
  const handleSaveEntry = (payload, editingId) => {
    ...
    if (editingId) {
      ...
      savedEntry = {
        ...
        recommendedActionsText: payload.recommendedActionsText,
        avoidActionsText: payload.avoidActionsText,
...
function ResultDetailForm({ data, entry, plantName, onSave, onCancel }) {
  ...
  return (
    <div className="eo-form">
      ...
      <div className="eo-related-hint">どちらか一方だけの記入でも構いません。</div>

      <div className="eo-form-actions">
        <button type="button" className="eo-btn eo-btn-ghost" onClick={onCancel}>
          キャンセル
        </button>
        <button type="button" className="eo-btn eo-btn-primary" onClick={() => onSave({ causes, goodResultText, badResultText })}>
          保存する
        </button>
      </div>
    </div>
