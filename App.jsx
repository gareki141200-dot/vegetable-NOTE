...
function DiagnoseScreen({ data }) {
    ...
    try {
      ...
      for (const en of withPhotos) {
        try {
          ...
          if (score >= MIN_MATCH) scored.push({ entry: en, score });
        } catch (err) {
  ...
  const aggregate = useMemo(() => {
    ...
    matches.forEach(({ entry: e }) => {
      (e.causes || []).forEach((c) => causeTally.set(c, (causeTally.get(c) || 0) + 1));
      if (e.actionText) {
    ...
    const causes = Array.from(causeTally.entries())
      .sort((a, b) => b[1] - a[1])
      .slice(0, 3)
      .map(([label]) => label);
    return {
  ...
  return (
    <div className="eo-diagnose">
      <div className="eo-diagnose-intro">
        写真を1枚選ぶと、これまでの記録の中から見た目が近い写真（一致度{MIN_MATCH}%以上）を探し、
        原因・対策・良かったこと・避けた方がよいことをまとめて表示します。APIやネット接続は使いません。
      </div>
              ...
              <>
                ...
                <div className="eo-diag-actions-list">
                  {aggregate.causes.map((c) => (
                    <span className="eo-tag-mini eo-tag-mini-cause" key={c}>
                      {c}
                    </span>
                  ))}
                </div>
